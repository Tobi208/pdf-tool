...
def parse_args(args: [str]):
    ...
    if not all(map(lambda s: type(s) == str, args)):
        raise TypeError('List of strings required!')
    # <action> <file1> are mandatory
    if len(args) < 2:
        raise ValueError('Missing <action> and/or <file1>!')
    action = args[0]
    if action not in actions:
        raise KeyError('Undefined <action>: ' + action + '!')
    ...
    if action in ['delete', 'extract', 'split']:
        ...
        """
        if len(xs) < 2:
            raise ValueError('Missing indices or ranges!')
    ...
    if action == 'insert':
        ...
        while i < len(xs):
            ...
            filei = xs[i]
            verify_file(filei, files)
            ts = (filei, )
            # iterate once or twice further to get <i> or <i> <j-k>
            j = i + 1
            if j >= len(xs):
                raise ValueError('Missing index to insert file: ' + xs[j - 1] + ' at!')
            r_i = list(get_range(xs[j]))
            ts += (r_i[0],)
            if len(r_i) != 1:
                raise TypeError('Index of where to insert file: ' + xs[j - 1] + ' at can not be a range!')
            j += 1
...
def get_range(s: str) -> range:
    ...
    if '-' in s:
        x = s.split('-')
        if len(x) != 2:
            raise ValueError('Expected format for indices or ranges: i or i-j!')
        i = int(x[0]) - 1
        j = int(x[1])
    else:
        i = int(s) - 1
    ...
    if i >= j or i < 0 or j < 1:
        raise ValueError('Range needs to be positive!')
    return range(i, j)
